...
import { Customer, CustomerSummary, archiveCustomer, createCustomer, listCustomersByName, listCustomersByEmail } from "./customers";
...
describe("List Customers By Email", () => {
  let mockServer: mockttp.Mockttp;
  const apiClient = new VendorPortalApi();
  apiClient.apiToken = "abcd1234";
  ...
  it("should return customers matching the email", async () => {
    const appId = "test-app-1";
    const appSlug = "test-app-1";
    const expectedApplications = {
      apps: [
    ...
    const customers: CustomerSummary[] = await listCustomersByEmail(apiClient, appSlug, "test@example.com");
    expect(customers).toHaveLength(2);
    expect(customers[0].name).toEqual("Test Customer");
    expect(customers[0].customerId).toEqual("customer-1");
    expect(customers[1].name).toEqual("Test Customer Two");
  ...
  it("should return empty array when no customers match", async () => {
    ...
    const customers: CustomerSummary[] = await listCustomersByEmail(apiClient, appSlug, "nonexistent@example.com");
    expect(customers).toHaveLength(0);
  });
  ...
  it("should properly handle email with special characters", async () => {
    const appId = "1234abcd";
    const appSlug = "app-1";
    const expectedApplications = {
      apps: [{ id: appId, name: "App 1", slug: appSlug }]
    ...
    const customers: CustomerSummary[] = await listCustomersByEmail(apiClient, appSlug, "test+tag@example.com");
    expect(customers).toHaveLength(1);
    expect(customers[0].name).toEqual("Customer & Co");
    expect(customers[0].customerId).toEqual("customer-1");
  });
  ...
  it("should throw error when API returns non-200 status", async () => {
    ...
    await expect(listCustomersByEmail(apiClient, appSlug, "test@example.com")).rejects.toThrow("Failed to list customers: Server responded with 500");
  });
  ...
  it("should search across all apps when appSlug is undefined", async () => {
    ...
    const customers: CustomerSummary[] = await listCustomersByEmail(apiClient, undefined, "test@example.com");
    expect(customers).toHaveLength(3);
    expect(customers[0].name).toEqual("Test Customer");
    expect(customers[1].name).toEqual("Test Customer Two");
    expect(customers[2].name).toEqual("Test Customer Three");
