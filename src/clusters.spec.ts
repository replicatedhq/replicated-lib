...
describe('ClusterService', () => {
  ...
  test('should return cluster', () => {
    ...
    return createCluster(apiClient, 'cluster1', 'kind', 'v1.25.1', '10m').then(
      (cluster) => {
...
describe('ClusterService use cases', () => {
  ...
  test('should return cluster with license_id', async () => {
    ...
    const cluster: Cluster = await createClusterWithLicense(
      apiClient,
      'cluster1',
      'embedded-cluster',
      '',
  ...
  test('upgrade a kurl cluster', async () => {
    ...
    await mockServer
      ...
      .thenReply(
        200,
        JSON.stringify({ cluster: { id: '1234abcd', status: 'upgrading' } })
      );

    const cluster: Cluster = await upgradeCluster(
      apiClient,
      '1234abcd',
      'latest'
    );
...
describe('pollForCluster', () => {
  ...
  test('should eventually return success with expected status', async () => {
    ...
    const cluster: Cluster = await pollForStatus(
      apiClient,
      '1234abcd',
      'running',
      1,
  ...
  test('should still fail on 404', async () => {
    ...
    await expect(
      pollForStatus(apiClient, '1234abcd', 'running', 1, 10)
...
describe('Cluster Add-ons', () => {
  ...
  test('should return object store add-on', async () => {
    ...
    const addon: Addon = await createAddonObjectStore(
      apiClient,
      clusterId,
      'test'
    );
  ...
  test('should eventually return success with expected status', async () => {
    ...
    const addon: Addon = await pollForAddonStatus(
      apiClient,
      '1234abcd',
      '1234abcd',
      'ready',
  ...
  test('should still fail on 404', async () => {
    ...
    await expect(
      pollForAddonStatus(apiClient, '1234abcd', '1234abcd', 'ready', 1, 10)
    ).rejects.toThrow(StatusError);
  });
});
...
describe('Cluster Exposed Ports', () => {
  ...
  test('should return exposed port', async () => {
    const clusterId = '1234abcd';
    const expectedExposedPort: ClusterPort = {
      upstream_port: 80,
      state: 'pending',
    ...
    const clusterPort: ClusterPort = await exposeClusterPort(
      apiClient,
      clusterId,
      80,
      ['http']
  ...
  test('should eventually return success with expected status', async () => {
    ...
    await mockServer
      ...
      .once()
      .thenReply(503);
    await mockServer.forGet(`/cluster/${clusterId}/ports`).thenReply(
      200,
      JSON.stringify({
    ...
    const clusterPort: ClusterPort = await pollForPortStatus(
      apiClient,
      '1234abcd',
      'http://mystifying-kepler.ingress.replicatedcluster.com/',
      'ready',
      1,
      10
    );
